...
| m5: restore                          | Restyle the existing divider and Restore button in their existing transcript position, at the saved transcript boundary. Use `Before this edit` as a caption; omit timestamps and the misleading after-edit placement/caption.              |
...
- Restore remains a rollback to its existing saved snapshot. Preserve its transcript boundary, IDs, and restored state. Implementation inspection found that when preprocessing proposes no edits, the subsequent main-query checkpoint already contains the user request; retain that existing behavior rather than moving the divider before it. Do not introduce dates or move the entry after analysis.
...
| `ChatWindow`          | Provider/Clear row, transcript, composer with input mount | Scope/warning and Clear row, transcript, styled input mount, permanent review container                 |
| `ChatInput`           | Form, textarea, Send                                      | The same form, textarea, and Send, restyled                                                             |

Systematic rules for the DOM move:

1. Construct the scope row, transcript, input mount, and review container once in `createInitialDom()` beneath the existing `ChatWindow` mount. Apply composer padding, background, and border directly to the input mount; omit the extra composer wrapper.
2. Keep the `ChatInput` instance and its mount throughout its lifetime. Hide/show the parent-owned input mount directly; never rebuild, reparent, or replace the child's form to show review actions.
...
- When a review entry exists, hide the input mount, show the action container, and show `Changes staged on <actual diff name> — original untouched`. Keep Clear chat in the row. When no review entry exists, hide/empty the action container, show the input mount, and restore the static scope caption.
