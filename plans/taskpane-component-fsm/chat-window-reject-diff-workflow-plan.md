...
Add `src/taskpane-fsm/pages/chat/chat-window-reject-diff-workflow.ts` and update `chat-window.ts` to construct and call the workflow. Give it the complete shared context and the existing `SubmitMessageWorkflow` instance.
...
When a rejected preprocessing diff must continue the original request, call `SubmitMessageWorkflow.run()` with the existing workflow ID and without duplicating the human message. Keep the dependency one-way.
