...
For local development, sideload the root `manifest.xml` and keep `npm start` running.
For a hosted deployment, publish the contents of `dist/` at the `PRODUCTION_URL` configured
in `vite.config.mts` and sideload `dist/manifest.xml` generated by `npm run build`.
If hosting elsewhere, update `PRODUCTION_URL` before building.
