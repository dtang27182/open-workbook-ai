...
let openrouterKeyStore: OpenrouterKeyStore;

export function configureOpenRouterClient(keyStore: OpenrouterKeyStore): void {
...
export async function requestOpenRouter(
  ...
  const response = await fetch("https://openrouter.ai/api/v1/responses", {
    method: "POST",
    headers: {
      Authorization: `Bearer ${openrouterKeyStore.get()}`,
...
export async function requestOpenRouterStream(
  ...
  let response: OpenRouterResponseBody | undefined;
...
export async function* requestOpenRouterStreamEvents(
  requestBody: OpenRouterRequestBody
