...
export class ChatStateMachine {
  ...
  constructor(
    private excelApi: ExcelApi | undefined,
    private view: ChatStateMachineUI
  ) {}
  ...
  async updateState(input: ChatStateMachineInput): Promise<void> {
    this.disableChatControls();
    try {
      this.validateInputForCurrentState(input);

      if (input.type === "submit_message") {
        await this.submitMessage(input.message);
      }
      ...
      if (input.type === "restore_to_point") {
        await this.restoreToPoint(input.restorePointId);
      }
    } catch (err) {
      console.debug(err);
      this.appendErrorMessage(this.getErrorMessage(input));
    } finally {
      this.configChatControls();
    }
  }
  ...
  private disableChatControls(): void {
    ...
    this.view.disableChatInputControls();
    this.renderChatTranscript();
  }
  ...
  private configChatControls(): void {
    ...
    this.renderChatTranscript();
    this.view.configChatControls(this.chatState.fsmState);
  }

  private renderChatTranscript(): void {
    this.view.renderTranscript(this.buildChatTranscript());
  }

  buildChatTranscript(): ChatTranscriptEntry[] {
  ...
  getCurrentTurnState(): ChatFsmState | undefined {
    return this.chatState.fsmState;
  }
